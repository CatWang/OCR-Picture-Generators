...
rootDir = '/alter/to_wangzh'

# Chinese fonts directory
fontDir = "/alter/fonts"
font_dir = os.walk(fontDir)

#Calculate how many fonts
...
#write the csv file
with codecs.open('big.csv', 'w', encoding='utf-8') as csvfile:
    csvfile.write('%s %s\n' % ('filename', 'lies'))
    ...
    while (hh < 30):
        ...
        # iterate the whole directory and find all the image files
        for root, d, fs in list_dir:
            for file in fs:
                ...
                # resize the image to guarantee that its width is enough for the characters that will be written
                width_after = 1200.0
                # get the height after resize
                height_after = width_after / width * height
                width_after = int(width_after)
                height_after = int(height_after)
                # if the height is not enough for three lines of character sequences, then abort it
                if height_after <= 300:
                    continue
                after = im.resize((int(width_after), int(height_after)), Image.ANTIALIAS)
                draw = ImageDraw.Draw(after)

                # randomly choose how many lines will be written in the current image
                lines = random.randint(1, 3)

                # randomly set the length of each line in the image
                length = random.randint(2, 6)
                ll = length
                line = 0

                # randomly choose a font from your font directory
                ...
                ttfont = ImageFont.truetype(path, font_size)

                # set a name for the result image
                filename = 'big/' + str(file_count) + '.jpg'

                # write the image name and how many lines of characters it contains in csv file
                csvfile.write('%s %s\n' % (filename, lines))
                file_count += 1

                # width used to set the maximum length of the three sequences,
                # this will be used to crop out the final image
                max_width = 0

                # width used to set the final height of the three sequences,
                # this will be used to crop out the final image
                max_heigh = 0
                up = 0

                # write character sequences line by line
                while line < lines:
                    line += 1
                    ...
                    # randomly choose characters from the letter array
                    while (length > 0):
                    ...
                    # randomly set the font color
                    r = random.randint(0, 254)
                    g = random.randint(0, 254)
                    b = random.randint(0, 254)

                    # draw the characters line by line
                    draw.text((2, up + 2), string.decode('utf-8'), fill=(r, g, b), font=ttfont)
                    print string

                    # get the approximate width and height of the current line
                    t_width, t_height = draw.textsize(unicode(string, 'UTF-8'), font=ttfont)

                    # write the (x,y) value of left-top and right-bottom points
                    csvfile.write('%s %s %s %s\n' % (0, up, 6 + t_width, up + t_height + 18))

                    # box = (0, up, 6 + t_width, up + t_height + 18)
                    # region = after.crop(box)
                    # fname = 'hhh/' + str(fcount) + '.jpg'
                    # fcount += 1
                    # region.save(fname)

                    # update the height where the next line can be draw
                    up = up + t_height + 18
                ...
                # crop out the final image
                box = (0, 0, max_width, max_heigh)
                region = after.crop(box)
